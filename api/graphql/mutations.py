...
class CreateAccount(graphene.Mutation):
    ...
    def mutate(root, info, account_data=None):
        try:
            user_by_username = Users.query.filter_by(username=account_data.username).first()
            if user_by_username:
                return CreateAccount(ok=False, message=MESSAGES.get("USER_EXIST"))

            user_by_email = Users.query.filter_by(email=account_data.email).first()
            if user_by_email:
...
class LoginUser(graphene.Mutation):
    ...
    def mutate(root, info, username_or_email, password):
        ...
        if user and user.valid_passkey(password):
            ...
            logged_in = True
            return LoginUser(logged_in=logged_in, message=message,
                             token=create_access_token(user.username, user_claims={"role": user.user_type}))
        else:
            message = MESSAGES.get("LOGIN_ERR")
            logged_in = False
            return LoginUser(logged_in=logged_in, message=message, token=None)
